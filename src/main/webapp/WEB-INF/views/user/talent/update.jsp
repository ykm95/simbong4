...
function getCheckParams() {
	...
	params = "${talent.talent_cycle}".split(",");
	console.log(params);
...
var checkList = getCheckParams();
	
//활동일자
$("#talent_start").prop("value","${talent.talent_start}");
$("#talent_end").prop("value","${talent.talent_end}");

//활동명
$("#talent_title").prop("value","${talent.talent_title}");

//봉사유형
$("input[type='radio'][name='talent_type'][value='${talent.talent_type}']").prop("checked", true);

// 봉사주기
for (var i = 0; i < checkList.length; i++) {
	$("input[type='checkbox'][name='t_cycle'][value='" + checkList[i] + "']").prop("checked", true);
}
...
<div class="container">
	...
	<form action="/user/talent/update" method="post" id="searchForm" style="padding: 30px 50px;">
	<input type="hidden" name="talentno" value="${talent.talentno }"/>
		...
		<option value="" selected>${talent.departname}</option>
		</select>
			</div>
		</div><br>
		...
		<select class="form-control" name="areano" id="areano" title="시군구" disabled="disabled" required="required">
			<option value="" selected>${talent.aname }</option>
		</select>
			</div>
		</div><br>
		...
		봉사주기
		<div class="form-group">
		<label><input name="t_cycle" id="t_cycleAll" type="checkbox" title="전체">&nbsp;전체</label>&nbsp;&nbsp;&nbsp;
		...
		<div>
		<textarea rows="500px" cols="400px" name="talent_content" id="editor1" >${talent.talent_content }</textarea>
		</div><br><br>
